...
export const SKILL_CATEGORIES: Record<SkillCategory, { label: string; stage: number; description: string }> = {
  ...
  script:    { label: '剧本扩写', stage: 0, description: '按类型定制扩写策略' },
  concept:   { label: '概念设定', stage: 1, description: '角色/场景一致性描述风格' },
  shotspec:  { label: '分镜设计', stage: 2, description: '景别/光位/构图偏好' },
  rendering: { label: '渲染出图', stage: 3, description: '灰模/渲染工作流参数' },
}
