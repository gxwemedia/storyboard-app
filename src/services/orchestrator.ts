...
字数根据原始大纲的信息密度和复杂度自适应调整，确保扩写充分但不冗余。`
...
export async function orchestrateStage1(
  ...
  const response = await sendPrompt(messages, { temperature: skill.temperature, timeoutMs: 300_000 })
...
输出严格的 JSON 数组，每个元素格式如下：
{
  ...
  "duration": "整数秒，根据镜头内容复杂度和叙事节奏决定（通常 2-8 秒）"
}
...
8. duration 为整数秒，根据镜头内容自行判断合理时长（快切动作 2-3 秒，叙事镜头 4-6 秒，情绪渲染可达 8 秒）
...
输出格式：直接输出一段连贯的中文描述，字数根据角色复杂度自适应（简单角色简短，复杂角色详尽），不要用列表，不要用 markdown。这段文本将直接嵌入后续的分镜 Prompt 中。`
...
输出格式：直接输出一段连贯的中文描述，字数根据场景复杂度自适应（简单场景简短，复杂场景详尽），不要用列表，不要用 markdown。这段文本将直接嵌入后续的分镜 Prompt 中。`
